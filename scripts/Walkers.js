...
const walkerCityArr = getWalkerCities();
const filterWalkerCitiesByWalker = (walkerObj) => {
    let newArr = [];
    for(const walkerCity of walkerCityArr) {
       if(walkerObj.id === walkerCity.walkerId) {
        newArr.push(walkerCity);
       }
    }
    return newArr;
}
...
const assignedCityNames = (walkerCityArr) => {
    let cityString = "";
